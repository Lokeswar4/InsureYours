...
```
CSV → ETL (etl_load.py) → MySQL → SQL Analytics → REST API (api.py)
                                 → Data Profiler (data_profiler.py)
                                 → Statistical Analysis (statistical_analysis.py)
```

### Pipeline stages:
1. **Schema** (`Tables_Creation.sql`) — Database, staging table, 5 dimension tables, Admission fact table, output table. Idempotent.
2. **ETL** (`etl_load.py`) — Validates rows, loads staging, populates dimensions, resolves FKs via 5-way JOIN.
3. **Core analysis** (`StoredProcedure.sql` + `Analysis.sql`) — View with age groups + LengthOfStay, 3 stored procedures, DENSE_RANK provider ranking.
4. **Advanced analysis** (`Advanced_Analysis.sql`) — Percentiles, outlier detection, market share, cost efficiency, readmission proxy, monthly trends.
5. **API** (`api.py`) — FastAPI REST endpoints serving recommendations and analytics.
6. **Profiling** (`data_profiler.py`) — 9-section data quality report with automated scoring.
7. **Statistics** (`statistical_analysis.py`) — Welch's t-test, ANOVA, confidence intervals, Cohen's d.

## Development Commands

```bash
make help                          # Show all commands
make test                          # 27 unit tests (no MySQL needed)
...
- `tests/test_etl.py` — 27 unit tests covering all validation logic
- Tests need only Python (no MySQL)
...
`ETL/` directory has the original Windows SSIS package (SQL Server + Visual Studio). Replaced by `etl_load.py`.
